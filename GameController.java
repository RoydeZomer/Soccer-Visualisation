...
public class GameController implements Drawable, Runnable {
  ...
  public void validateData() {
    ...
    int ballerrors = 0;
    for (int frame = 1; frame < frames; frame++) {
      double ballSpeedMPS = calculateDistance(ballpositions[frame-1], ballpositions[frame])*25;//mps
      if (ballSpeedMPS >= 40) {
        //System.out.println("wrong ball data at frame: "+frame + " With ball speed in MPS: " + ballSpeedMPS);
        ballerrors++;
      }
      for (int player = 0; player < 22; player++) {
        double walkingSpeedMPS = calculateDistance(playersPos[player][frame-1], playersPos[player][frame])*25;//mps
